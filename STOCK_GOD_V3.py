...
class TaiwanStockTradingSystem:
    def __init__(self, tickers, start_date="2023-01-01"):
        ...
        self.buy_fee_rate = 0.001425 * 0.5
        self.sell_fee_rate = 0.001425 * 0.5
        self.sell_tax_rate = 0.003
    ...
    def simulate_strategy(self, df):
        df = df.copy()
        df['Exec_Buy_Signal'] = df['Buy_Signal'].shift(1).fillna(False).astype(bool)
        df['Exec_Sell_Signal'] = df['Sell_Signal'].shift(1).fillna(False).astype(bool)
        df['Position'] = 0
        df['Trade_Action'] = 0.0
        df['Trade_Cost'] = 0.0
        ...
        for i in range(1, len(df)):
            idx = df.index[i]
            daily_return = float(df['Close'].iloc[i] / df['Close'].iloc[i - 1] - 1)

            if in_position:
                hold_days += 1
                highest_close = max(highest_close, float(df['Close'].iloc[i]))
                atr = float(df['ATR14'].iloc[i]) if not pd.isna(df['ATR14'].iloc[i]) else 0.0
                trailing_stop = highest_close - (2.5 * atr) if atr > 0 else np.nan
                entry_stop = entry_price - (2.0 * atr) if atr > 0 else entry_price * 0.93
                stop_price = max(entry_stop, trailing_stop) if not pd.isna(trailing_stop) else entry_stop
                stop_hit = float(df['Close'].iloc[i]) < stop_price
                time_stop = hold_days >= 10 and (float(df['Close'].iloc[i]) / entry_price - 1) < 0.03
                can_exit = hold_days >= self.min_holding_days
                exit_signal = bool(df['Exec_Sell_Signal'].iloc[i]) or stop_hit or time_stop

                if exit_signal and can_exit:
                    gross_return = float(df['Close'].iloc[i]) / entry_price - 1
                    round_trip_cost = self.buy_fee_rate + self.sell_fee_rate + self.sell_tax_rate + (self.slippage_rate * 2)
                    net_return = gross_return - round_trip_cost
                    df.at[idx, 'Trade_Action'] = -1.0
                    df.at[idx, 'Trade_Cost'] = self.sell_fee_rate + self.sell_tax_rate + self.slippage_rate
                    df.at[idx, 'Strategy_Returns'] = daily_return - df.at[idx, 'Trade_Cost']
                    ...
                    df.at[idx, 'Strategy_Returns'] = daily_return
                    df.at[idx, 'Entry_Price'] = entry_price
                    df.at[idx, 'Hold_Days'] = hold_days
                    df.at[idx, 'Trailing_Stop'] = stop_price
            elif bool(df['Exec_Buy_Signal'].iloc[i]):
                in_position = True
                ...
                df.at[idx, 'Position'] = 1
                df.at[idx, 'Trade_Action'] = 1.0
                df.at[idx, 'Trade_Cost'] = self.buy_fee_rate + self.slippage_rate
                df.at[idx, 'Strategy_Returns'] = -df.at[idx, 'Trade_Cost']
                df.at[idx, 'Entry_Price'] = entry_price
                df.at[idx, 'Hold_Days'] = hold_days

            if in_position and entry_idx is not None and df.at[idx, 'Position'] == 0:
                df.at[idx, 'Position'] = 1

        return df, pd.DataFrame(trade_returns)
    ...
    def calculate_indicators(self, df):
        ...
        df['ATR14'] = self.calculate_atr(df, window=14)
    ...
    def process_stock(self, ticker):
        ...
        daily_chase_risk = df['Returns'] >= 0.06
        trend_filter = (df['Close'] > df['MA20']) | df['Independent_Alpha'] | df['Pro_Bottom_Breakout'] | df['Ambush_Setup'] | df['Fake_Break']
        df['Buy_Signal'] = (df['Score'] >= 60) & trend_filter & (~daily_chase_risk)
        ...
        df, trade_df = self.simulate_strategy(df)
    ...
    def run_analysis(self):
        ...
        for ticker in self.tickers:
            ...
            trade_df = df.attrs.get('trade_df', pd.DataFrame())
            closed_trades = len(trade_df)
            trades = range(closed_trades)
            win_rate = (trade_df['net_return'] > 0).mean() if closed_trades > 0 else 0
            avg_trade_return = trade_df['net_return'].mean() if closed_trades > 0 else 0
            payoff_ratio = (
                trade_df.loc[trade_df['net_return'] > 0, 'net_return'].mean() /
                abs(trade_df.loc[trade_df['net_return'] <= 0, 'net_return'].mean())
                if closed_trades > 0 and (trade_df['net_return'] > 0).any() and (trade_df['net_return'] <= 0).any()
                else 0
            )
            ...
            results_summary[ticker].update({
                "平均每筆 (%)": round(avg_trade_return * 100, 2),
                "賺賠比": round(float(payoff_ratio), 2),
...
#    DYNAMIC_MAP = {f"{item['code']}.TW": item['name'] for item in hot_stocks}
    ...
    for stock, data in summary.items():
        if stock in STATIC_YF_MAP:
            print(f"🔹 {stock} {COMBINED_MAP.get(stock, ''):<4} | 勝率: {data['勝率 (%)']:>5}% | 總報酬: {data['策略累積報酬 (%)']:>6}% | 均筆: {data.get('平均每筆 (%)', 0):>5}% | MDD: {data.get('最大回撤 (%)', 0):>6}%")
